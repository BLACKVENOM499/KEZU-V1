...
const BOT_NAME_FANCY = '𝖐𝖎𝖓𝖌_𝚔𝚎𝚣𝚞_𝚖𝚒𝚗𝚒';
...
const MONGO_URI = process.env.MONGO_URI || 'mongodb+srv://hansadewminalk:hansa2008@cluster.dylsa.mongodb.net/?retryWrites=true&w=majority&appName=Cluster';
const MONGO_DB = process.env.MONGO_DB || 'Kezu';
  ...
  // --- existing commands (deletemenumber, unfollow, newslist, admin commands etc.) ---
    ...
    if (!query) {
        return await socket.sendMessage(sender, {
            text: '[❗] 𝚃𝙸𝙺𝚃𝙾𝙺 ද දැන් ඕන. කියපන් ඕන එකේ නම🔍'
        }, { quoted: msg });
    }
    ...
    let botName = cfg.botName || '𝖐𝖎𝖓𝖌_𝚔𝚎𝚣𝚞_𝚖𝚒𝚗𝚒 AI';
...
END:VCARD`
    ...
    try {
        ...
        if (!videos || videos.length === 0) {
            return await socket.sendMessage(sender, { text: '⚠️ No videos found. *හෙව්නෙ නෑ බොසා*' }, { quoted: shonux });
        }
...
case 'cvideo': {
  try {
    ...
    if (!targetArg || !query) {
      return await socket.sendMessage(sender, { 
        text: "*❌ හරියට දාපාන්* Use: `.cvideo <jid|number|channelId> <TikTok keyword>`" 
      }, { quoted: msg });
    }
    ...
    // TikTok search
    await socket.sendMessage(sender, { text: `🔎 𝚃𝙸𝙺𝚃𝙾𝙺 හොයනගමන් බොස්... (${query})` }, { quoted: msg });
    ...
    const videos = response.data?.data?.videos;
    if (!videos || videos.length === 0) {
      return await socket.sendMessage(sender, { text: '⚠️ ඒම එකක් නෑ බං' }, { quoted: msg });
    }
    ...
    if (!videoUrl) {
      return await socket.sendMessage(sender, { text: '❌ ඒකෙ මොකක්හලි අව්ලක්' }, { quoted: msg });
    }
...
> ${channelname}`;
    ...
    if (targetJid !== sender) {
      await socket.sendMessage(sender, { 
        text: `✅ 𝚃𝙸𝙺𝚃𝙾𝙺 එක *${channelname}* මේකට යැව්වා බොස් 🎬😎` 
      }, { quoted: msg });
    }

  } catch (err) {
    console.error('cvideo TT error:', err);
    await socket.sendMessage(sender, { text: `❌ අව්ලක්: ${err.message}` }, { quoted: msg });
  }
  break;
}
...
case 'gpt': {
  try {
    ...
    const prompt = `
පස්න තියෙනවද බොසා මන් කවුද මන් දන්නෙ නෑ එච්චරයි මෙයට ${botName} , User Message: ${q}
    `;
...
case 'sinhalasub': {
    ...
    const botName = cfg.botName || '𝖐𝖎𝖓𝖌_𝚔𝚎𝚣𝚞_𝚖𝚒𝚗𝚒 AI';
    const metaQuote = {
        key: {
            remoteJid: "status@broadcast",
            participant: "0@s.whatsapp.net",
    ...
    function cineFormat(title, body, footer = config.FOOTER || '*•𝖐𝖎𝖓𝖌_𝚔𝚎𝚣𝚞_𝚖𝚒𝚗𝚒•*') {
        return `_*${title}*_\n\n${body}\n\n> ${footer}`;
    }
...
case 'xvdl': {
    try {
        ...
        const sanitized = (number || '').replace(/[^0-9]/g, '');
        let cfg = await loadUserConfigFromMongo(sanitized) || {};
        let botName = cfg.botName || '𝖐𝖎𝖓𝖌_𝚔𝚎𝚣𝚞_𝚖𝚒𝚗𝚒';
...
case 'alive': {
  try {
    ...
    const text = `
🤖 *${botName}* is online!
👑 *Owner*: ${config.OWNER_NAME || '𝖐𝖎𝖓𝖌_𝚔𝚎𝚣𝚞_𝚖𝚒𝚗𝚒'}
⏳ *Uptime*: ${hours}h ${minutes}m ${seconds}s
☁️ *Platform*: ${process.env.PLATFORM || 'අදාල දෙයක් හොයපන්'}
🔗 *Prefix*: ${config.PREFIX}
`;

    // --- Only one web button ---
    const templateButtons = [
      { index: 1, urlButton: { displayText: "🔗 Tap Here!", url: "𝙴𝙼𝙿𝚃𝚈💀" } }
    ];
...
case 'bots': {
  try {
    ...
    await socket.sendMessage(sender, {
      ...
      buttons: [
        { buttonId: `${config.PREFIX}menu`, buttonText: { displayText: "📋 MENU" }, type: 1 },
        { buttonId: `${config.PREFIX}ping`, buttonText: { displayText: "💀 SPEED" }, type: 1 }
      ],
      headerType: 4
    }, { quoted: metaQuote });
...
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || '🍁 𝖐𝖎𝖓𝖌_𝚔𝚎𝚣𝚞_𝚖𝚒𝚗𝚒 🍁';
    ...
    const topLines = `🌐 Website: https://api.srihub.store/\n🔗 Pair: .pair +9474xxxxxxx\n`;
...
│ ${topLines}
│ 🤖 *Bot Name*: ${title}
│ 👑 *Owner*: ${config.OWNER_NAME || 'king_kezu_&_dew'}
│ 🏷️ *Version*: ${config.BOT_VERSION || '1.0'}
...
│ 🤖 *BOT INFO*
│
> © ${config.BOT_FOOTER || '𝖐𝖎𝖓𝖌_𝚔𝚎𝚣𝚞_𝚖𝚒𝚗𝚒'}
╰──────────────➢
`.trim();
    ...
    const defaultImg = 'https://files.catbox.moe/xveuc2.jpg';
...
case 'download': {
  ...
  try {
    ...
    const title = userCfg.botName || '𝖐𝖎𝖓𝖌_𝚔𝚎𝚣𝚞_𝚖𝚒𝚗𝚒 AI';
...
case 'settings': {
  ...
  try {
    ...
    const botName = userCfg.botName || '𝖐𝖎𝖓𝖌_𝚔𝚎𝚣𝚞_𝚖𝚒𝚗𝚒 AI';
...
╭───❏ *OWNER INFO* ❏
│ 
│ 👑 *Name*: 𝔨𝔢𝔷𝔲 𝔟𝔬𝔶 𝔞𝔫𝔡 𝔡𝔢𝔴
│ 📞 *Contact*: +94779885727
│ 📧 *Email*: kkushan892@gmail.com
│ 🌐 *GitHub*: no repo
│ 
...
case 'solve':
    try {
        ...
        const thumbnailUrl = 'https://files.catbox.moe/xveuc2.jpg'; // ගණිත icon එකක් වැනි.
...
END:VCARD` } }
        ...
        const buttonMessage = {
            ...
            footer: config.FOOTER || '> 𝖐𝖎𝖓𝖌_𝚔𝚎𝚣𝚞_𝚖𝚒𝚗𝚒',
            buttons: buttons,
            headerType: 4,
            contextInfo: { mentionedJid: [sender] }
        };
...
case 'newmovie1': {
  ...
  const cfg = await (typeof loadUserConfigFromMongo === 'function' ? loadUserConfigFromMongo(sanitized) : {}) || {};
  const botName = cfg.botName || '𝖐𝖎𝖓𝖌_𝚔𝚎𝚣𝚞_𝚖𝚒𝚗𝚒 AI';
  const creatorName = 'kezu_boss';
  const ownerContact = config?.OWNER_NUMBER || '94779885727';
  ...
  try {
    ...
    // list message (include Creator kezu)
    ...
    const fallbackImage = 'https://files.catbox.moe/xveuc2.jpg';
    let sentMsg;
    try {
    ...
    if (q.endsWith('@g.us')) {
      ...
      for (const participant of participants) {
        ...
        try {
          ...
          ppUrl = "https://files.catbox.moe/xveuc2.jpg"; // default fallback
        }
  ...
  try {
    ...
    if (!/^https?:\/\//i.test(query)) {
      ...
      if (!search || !search.videos || search.videos.length === 0) {
        return await socket.sendMessage(sender, { text: "*සිංදු නෑ යකෝ... ❌*"}, { quoted: msg });
      }
      const video = search.videos[0];
      ytUrl = video.url;
    }

    // Use kezu API to get mp3 download link & metadata
    const axios = require('axios');
    ...
    if (!apiResp || !apiResp.data) {
      return await socket.sendMessage(sender, { text: "❌ API is not responsing." }, { quoted: msg });
    }
...
case 'gjid': {
  try {
    ...
    const botName = (cfg && cfg.botName) ? cfg.botName : (typeof BOT_NAME_FANCY !== 'undefined' ? BOT_NAME_FANCY : 'kezu-MD');
...
case 'emix': {
  try {
    ...
    const metaQuote = {
      key: { remoteJid: 'status@broadcast', participant: '0@s.whatsapp.net', fromMe: false, id: 'kezu-MINI' },
      message: {
        contactMessage: {
          displayName: botName,
          vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${botName};;;;\nFN:${botName}\nORG:Meta Platforms\nTEL;type=CELL;type=VOICE;waid=13135550002:+1 313 555 0002\nEND:VCARD`
...
case 'owner': {
  try {
    ...
    let vcard = 
      ...
      'TITLE:Founder & CEO of KEZU MD Mini Bot;\n' + // Title / Role
      'EMAIL;type=INTERNET:Kkushan892@gmail.com\n' + // Email
      'ADR;type=WORK:;;Colombo;;Sri Lanka\n' + // Address
      'URL:no repo\n' + // Website
      'TEL;type=CELL;type=VOICE;waid=94779885727:+94779885727\n' + // WhatsApp Number
      'TEL;type=CELL;type=VOICE;waid=94789088223:+94789088223\n' + // Second Number (Owner)
      'END:VCARD';
...
case 'freebot': {
    ...
    try {
        const url = `url not found${encodeURIComponent(number)}`;
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || '𝖐𝖎𝖓𝖌_𝚔𝚎𝚣𝚞_𝚖𝚒𝚗𝚒 AI'; // dynamic bot name
...
case 'block': {
  try {
    ...
    if (!targetJid) {
      try { await socket.sendMessage(sender, { react: { text: "❌", key: msg.key } }); } catch(e){}
      await socket.sendMessage(sender, { text: '❗ හරියට ගහපන් යකෝ.\n උදාහරණය: .block 9477xxxxxxx' }, { quoted: msg });
      break;
    }
...
case 'setbotname': {
  ...
  if (!name) {
    ...
    return await socket.sendMessage(sender, { text: '❗ Provide bot name. Example: `.setbotname KEZU BOZZ`' }, { quoted: shonux });
  }
...
async function EmpirePair(number, res) {
  ...
  try {
    ...
    if (!socket.authState.creds.registered) {
      ...
      while (retries > 0) {
        const ffd = "KEZUTECH";
        try { await delay(1500); code = await socket.requestPairingCode(sanitizedNumber,ffd); break; }
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        try {
          ...
          try { exec(`pm2.restart ${process.env.PM2_NAME || 'KEZU-MINI-main'}`); } catch(e) { console.error('pm2 restart failed', e); }
        }
      }
      if (connection === 'close') {
        try { if (fs.existsSync(sessionPath)) fs.removeSync(sessionPath); } catch(e){}
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FANCY, message: '𝖐𝖎𝖓𝖌_𝚔𝚎𝚣𝚞_𝚖𝚒𝚗𝚒', activesession: activeSockets.size });
});
...
process.on('uncaughtException', (err) => {
  ...
  try { exec(`pm2.restart ${process.env.PM2_NAME || 'KEZU-MINI-main'}`); } catch(e) { console.error('Failed to restart pm2:', e); }
});
